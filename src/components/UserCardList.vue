<template>
  <div>
    <van-skeleton v-for="user in userList" :key="user" :loading="props.loading" :row="3" avatar title
    >
      <van-card
      ...
      >
        <template #tags>
          <van-tag v-for="tag in user.tags" :key="tag" plain style="margin-right: 8px;margin-top: 8px" type="danger">{{
              tag
            }}
          </van-tag>
        </template>
    ...
    <van-popup v-model:show="show">
      ...
      <van-divider/>
      <van-cell :value="clickUser.planetCode" center title="伙伴学号:"/>
